    ...
    return (
        <div className="h-full w-full z-0">
            <MapContainer
                ...
                minZoom={6}
                maxBounds={[
                    [18.0, 85.0], // Much looser bounds to allow popups to fit
                    [30.0, 96.0]  // Extended North significantly
                ]}
                maxBoundsViscosity={0.5} // Allow elastic panning
            ...
            >
                ...
                {Object.entries(groupedEvents).map(([key, group]) => {
                    ...
                    // Calculate days ago for worst event
                    const publishDate = new Date(worstEvent.publishedAt)
                    const now = new Date()
                                                ...
                                                {group.map((event, idx) => (
                                                    <div key={event.id} className="border-b last:border-0 pb-3 border-gray-100">
                                                        ...
                                                        <div className="flex gap-3 text-xs mb-1">
                                                            ...
                                                            <span className="text-gray-500">{new Date(event.publishedAt).toLocaleDateString()}</span>
                                                        </div>
                                                        {event.summary && (
