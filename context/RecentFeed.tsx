...
function EventItem({ event }: { event: Event }) {
    ...
    try {
        ...
        if (event.additionalSources) extraSources = JSON.parse(event.additionalSources)
    } catch (e) { }
    ...
    return (
        <div className="block p-6 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
            ...
            {/* Summary */}
            {event.summary && (
                <p className="text-sm text-gray-700 dark:text-gray-300 mb-4 line-clamp-2 bg-yellow-50 dark:bg-yellow-900/10 border-l-2 border-yellow-400 dark:border-yellow-600 p-3 rounded">
                    {event.summary}
                </p>
            )}
                            ...
                            <div className="space-y-1 pl-1">
                                {extraSources.map((s, idx) => (
                                    <div key={idx} className="flex items-center justify-between">
                                        <span className="text-gray-500 dark:text-gray-500">- {s.source || 'Additional Source'}</span>
                                        <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline flex items-center gap-1">
                                            Link <ExternalLink className="w-[10px] h-[10px]" />
                                        </a>
                                    </div>
                                ))}
                            </div>
