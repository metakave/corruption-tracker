...
export function geocodeLocation(locationText: string): { lat: number; lng: number; district: string } | null {
    ...
    if (SEARCH_INDEX.has(input)) {
        const key = SEARCH_INDEX.get(input)!
        if (bangladeshDistricts[key]) {
            return { ...bangladeshDistricts[key], district: key }
        }
    }

    // 2. Partial match (O(N)) - Check if input contains any known district/upazila term
    // We prioritize strictness. If the input *contains* "Savar", we map to Dhaka.
    ...
    // 3. Last resort: Check keys of bangladeshDistricts directly (Original Logic)
