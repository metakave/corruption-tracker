...
// BdNews24Scraper removed due to persistent Cloudflare blocking (0 article yield)
...
async function main() {
    ...
    try {
        // Order: Ajker Patrika -> Jugantor -> Samakal -> Dhaka Post -> Prothom Alo
        // Disabled: Ittefaq (slow/timeouts), News24BD (low yield), BdNews24 (Cloudflare)
        const scrapers = [
            // new IttefaqScraper(), // Disabled: Persistent remote timeouts
            new AjkerPatrikaScraper(),
            new JugantorScraper(),
            // new News24BDScraper(), // Disabled: Low yield & limited metadata
            new SamakalScraper(),
        ...
        for (const scraper of scrapers) {
            try {
                ...
                const articles = await scraper.scrape(new Date('2026-06-09T00:00:00Z'))
        ...
        for (const article of allArticles) {
            try {
                // Validate Date before finding/saving
                if (!article.publishedAt || isNaN(new Date(article.publishedAt).getTime())) {
                    console.warn(`⚠️ Skipping article with invalid date: "${article.title}" (${article.url})`)
                    continue;
                }
