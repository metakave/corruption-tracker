        ...
        fetch('/api/events')
            ...
            .then(json => {
                ...
                events.forEach((e: any) => {
                    const textToSearch = (e.politicalParties || '') + ' ' + (e.title || '') + ' ' + (e.summary || '')
                    let matched = false

                    for (const cat of PARTY_CATEGORIES) {
                        const isMatch = cat.keywords.some(k => textToSearch.includes(k))
                        if (isMatch) {
                            categoryCounts[cat.label] = (categoryCounts[cat.label] || 0) + 1
                            matched = true
                        }
                    }
